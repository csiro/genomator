...
# Ported from experiment/experiment_tools/tools/WGAN_run.py unchanged except
# for: the import line below (baseline_vcf_io replaces the cyvcf2-based
# experiment_tools loader - see that module's docstring); dropping the
# `import tensorflow as tf` / `from tensorflow.keras.optimizers.legacy import
# Adam` lines further down, which the original file never actually used
# (main_WGAN builds its optimizers with torch.optim.Adam, not the imported
# Keras one) - removing a dead import isn't a model/training change, and it
# saves everyone who only wants crbm/rbm/markov/genomator from needing
# TensorFlow installed too. This file is imported lazily (only inside
# providers.wgan_generate, after checking torch is installed) so providers.py
# stays importable without PyTorch.
#
# The GAN architecture hardcodes latent_depth_factor=14, i.e.
# latent_size = int((Nv+1)/2**14) where Nv = variant_count*ploidy - see the
# comment in main_WGAN ("14 for 65535 SNP data"). For latent_size to be >=1
# (required for the conv stack to have any width at all) Nv must be at least
# ~16384, i.e. roughly 8192+ variants at ploidy 2. This is a hard limit of
# the ported architecture, not something providers.wgan_generate works
# around - a VCF with too few variants will fail inside main_WGAN with a
# shape error, regardless of epoch/batch-size settings.
#
# The model and training code (ConvGenerator, ConvDiscriminator, main_WGAN's
# training loop) is otherwise untouched. Note main_WGAN writes pickled genome
# strings, not VCF, at "{output_vcf_file}{epoch}.pickle" checkpoints -
# providers.wgan_generate converts the one dumped checkpoint to a real VCF
# afterwards, the same way experiment/experiment_tools/tools/pickle_to_vcf.py
# does.
