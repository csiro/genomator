...
# Ported from experiment/experiment_tools/tools/CRBM_run.py unchanged except
# for the import line below (baseline_vcf_io replaces the cyvcf2-based
# experiment_tools loader - see that module's docstring) and this file is
# imported lazily (only inside providers.crbm_generate, after checking torch
# is installed) so providers.py stays importable without PyTorch. The model
# and training code (the CRBM class, fit()) is untouched. Note main_CRBM's
# fit() writes pickled genome strings, not VCF, at
# "{output_vcf_file}{epoch}.pickle" checkpoints - providers.crbm_generate
# converts the highest-epoch one to a real VCF afterwards, the same way
# experiment/experiment_tools/tools/pickle_to_vcf.py does.
