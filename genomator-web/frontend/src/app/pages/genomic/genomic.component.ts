...
export class GenomicComponent implements OnInit {
  ...
  ngOnInit() {
    this.form = new FormGroup({
      ...
      exception_space: new FormControl(0, Validators.compose([Validators.required])),
      looseness: new FormControl(0, Validators.compose([Validators.required])),
  ...
  handlePick(e: Event) {
    ...
    if (!file.name.toLowerCase().endsWith(".vcf")) { alert('File extension did not match .vcf'); return; }
    this.file = file;
  }
  ...
  setProgressString(s: string) {
    ...
    if (s.startsWith("loaded variants ")) {
      ...
      if (match) this.progress1 = parseInt(match[1], 10)*100.0/parseInt(match[2], 10);
    }
    if (s.startsWith("cluster distance iteration ")) {
      match = s.match(/^cluster distance iteration (\d+)\/(\d+)$/);
      if (match) this.progress2 = parseInt(match[1], 10)*100.0/parseInt(match[2], 10);
    }
    if (s.startsWith("cluster re run ")) {
      match = s.match(/^cluster re run (\d+)\/(\d+)$/);
      if (match) this.progress3 = parseInt(match[1], 10)*100.0/parseInt(match[2], 10);
    }
    if (s.startsWith("Completed ")) {
      match = s.match(/^Completed (\d+)\/(\d+)$/);
      if (match) this.progress4 = parseInt(match[1], 10)*100.0/parseInt(match[2], 10);
    }
    if (s.startsWith("output records ")) {
      match = s.match(/^output records (\d+)\/(\d+)$/);
      if (match) this.progress5 = parseInt(match[1], 10)*100.0/parseInt(match[2], 10);
    }
  }
  ...
  async submitJob() {
    ...
    let exception_space = this.form.value.exception_space;
    let looseness = this.form.value.looseness;
    ...
    this.pyodideService.loadFile(new Uint8Array(await this.file.arrayBuffer()), "vcf_input.vcf");
    this.pyodideService.execute(
      "Genomator_exec",["vcf_input.vcf","vcf_output.vcf",number_of_data, -exception_space, cluster_group_size,looseness]
    ).then (() => {
  ...
  loadExample() {
    if (this.loading == false) {
      this.disableSubmit = true;
      this.disableDownload = true;
      this.http.get("assets/805_SNP_1000G_real.vcf", { responseType: 'arraybuffer' }).subscribe(
        (data: ArrayBuffer) => {
